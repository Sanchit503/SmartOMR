...
from .flow import FlowResult, LayoutTooTight, MCQEntry, WrittenEntry, flow_sheet
from .metrics import (
    BTECH_GRID_COLUMNS,
    BTECH_GRID_X_MM,
    BUBBLE_RADIUS_MM,
...
def _total_marks(config) -> float:
    return config.num_mcq * config.marks_per_mcq + sum(w.max_marks for w in config.written_questions)
